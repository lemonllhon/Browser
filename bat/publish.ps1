...
function Build-WindowsBinary {
    ...
    if (-not (Test-Path -LiteralPath $binaryPath -PathType Leaf)) {
        throw "构建产物不存在: build\bin\trace-browser.exe"
    }
    Write-Host "✓ 构建成功: build\bin\trace-browser.exe"
    Write-Host ""
}
...
function New-WindowsStaging {
    ...
    Copy-Item -LiteralPath $binaryPath -Destination (Join-Path $stagingDir "trace-browser.exe") -Force
    if (-not (Test-Path -LiteralPath (Join-Path $stagingDir "trace-browser.exe") -PathType Leaf)) {
        throw "staging 中缺少 trace-browser.exe"
    }
    Write-Host "✓ 复制 trace-browser.exe"
...
try {
    Write-Section "Trace Browser - 发布打包脚本"
    Write-Host ""
    ...
    if ($script:WindowsDone) {
        Write-Host "Windows 安装包: publish\output\TraceBrowser-Setup-$script:Version.exe"
    }
    if ($script:LinuxDone) {
