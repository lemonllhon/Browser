...
export function BrowserDetailPage() {
  ...
  const handleSwitchProxyNow = async () => {
    setPendingAction('switchingProxy')
    try {
      const updatedProfile = await switchBrowserProfileProxyNow(profile.profileId)
      if (updatedProfile) {
  ...
  const isSwitchingProxy = pendingAction === 'switchingProxy'
  const isBusy = pendingAction !== null
  const runtimeStatus = resolveRuntimeStatus(profile.running, profile.debugReady)
  const currentSwitchProxyName = profile.autoProxySwitchLastProxyId ? proxyNames[profile.autoProxySwitchLastProxyId] || profile.autoProxySwitchLastProxyId : ''
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card title="配置摘要" subtitle="指纹与启动参数">
              <div className="space-y-3 text-sm text-[var(--color-text-secondary)]">
                ...
                {profile.autoProxySwitchEnabled && (
                  <>
                    <div className="flex justify-between">
                      <span>代理切换模式</span>
                      <span>{profile.autoProxySwitchMode === 'manual' ? '仅手动切换' : '定时轮询切换'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>轮询分组</span>
                      <span>{profile.autoProxySwitchGroupName || '全部代理池节点'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>当前出口</span>
                      <span>{currentSwitchProxyName || '启动时随机选择'}</span>
                    </div>
                  </>
                )}
                <div className="flex justify-between">
          ...
          <Card title="快捷操作" subtitle="快速控制实例">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {profile.running && profile.autoProxySwitchEnabled && (
                <Button size="sm" variant="secondary" onClick={handleSwitchProxyNow} loading={isSwitchingProxy} disabled={isBusy && !isSwitchingProxy}>
                  {!isSwitchingProxy && <RefreshCw className="w-4 h-4" />}
                  {isSwitchingProxy ? '切换中' : '手动切换出口'}
                </Button>
              )}
            </div>
          </Card>
