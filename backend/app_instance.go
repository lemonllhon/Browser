...
func (a *App) browserInstanceStartInternal(profileId string, extraLaunchArgs []string, startURLs []string, skipDefaultStartURLs bool, preferVisibleWindow bool) (*BrowserProfile, error) {
	...
	if profile.AutoProxySwitchEnabled {
		...
		if switchErr != nil {
			...
			log.Error("代理自动切换中转启动失败",
				...
				logger.F("mode", profile.AutoProxySwitchMode),
	...
	} else if !autoSwitchProxy && proxy.RequiresBridge(resolvedProxyConfig, proxies, profile.ProxyId) {
...
func hasLaunchArgValue(args []string, target string) bool {
	target = strings.ToLower(strings.TrimSpace(target))
	if target == "" {
		return false
	}
	for _, arg := range args {
		if strings.ToLower(strings.TrimSpace(arg)) == target {
			return true
		}
	}
	return false
