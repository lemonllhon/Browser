...
`
	...
	if cfg.App.Name != "Trace Browser" {
		t.Fatalf("App.Name 未补齐: got=%q", cfg.App.Name)
	}
	if cfg.App.MaxProfileLimit != DefaultMaxProfileLimit {
		t.Fatalf("MaxProfileLimit 应迁移为无限制: got=%d want=%d", cfg.App.MaxProfileLimit, DefaultMaxProfileLimit)
	}
...
`
	...
	if cfg.App.Name != "Custom App" || cfg.App.MaxProfileLimit != DefaultMaxProfileLimit {
