...
func (m *Manager) loadProfiles() {
	...
	for _, item := range m.Config.Browser.Profiles {
		...
		m.Profiles[profileId] = &Profile{
			...
			AutoProxySwitchMode:          normalizeAutoProxySwitchMode(item.AutoProxySwitchMode),
...
func (m *Manager) SaveProfiles() error {
	...
	for _, profile := range m.Profiles {
		profiles = append(profiles, ProfileConfig{
			...
			AutoProxySwitchMode:          normalizeAutoProxySwitchMode(profile.AutoProxySwitchMode),
...
func (m *Manager) Create(input ProfileInput) (*Profile, error) {
	...
	if m.Config.App.MaxProfileLimit > 0 && len(m.Profiles) >= m.Config.App.MaxProfileLimit {
		return nil, fmt.Errorf("实例数量已达上限 (%d个)，无法创建新的实例", m.Config.App.MaxProfileLimit)
	}
	...
	profile := &Profile{
		...
		AutoProxySwitchGroupName:     strings.TrimSpace(input.AutoProxySwitchGroupName),
		AutoProxySwitchMode:          normalizeAutoProxySwitchMode(input.AutoProxySwitchMode),
		AutoProxySwitchIntervalM:     normalizeAutoProxySwitchInterval(input.AutoProxySwitchIntervalM),
		AutoProxySwitchRotateByGroup: input.AutoProxySwitchRotateByGroup && strings.TrimSpace(input.AutoProxySwitchGroupName) == "",
...
func (m *Manager) Update(profileId string, input ProfileInput) (*Profile, error) {
	...
	profile.AutoProxySwitchEnabled = input.AutoProxySwitchEnabled
	profile.AutoProxySwitchGroupName = strings.TrimSpace(input.AutoProxySwitchGroupName)
	profile.AutoProxySwitchMode = normalizeAutoProxySwitchMode(input.AutoProxySwitchMode)
	profile.AutoProxySwitchIntervalM = normalizeAutoProxySwitchInterval(input.AutoProxySwitchIntervalM)
	profile.AutoProxySwitchRotateByGroup = input.AutoProxySwitchRotateByGroup && profile.AutoProxySwitchGroupName == ""
	if profile.ProxyId != "" {
...
func (m *Manager) Copy(profileId string, newName string) (*Profile, error) {
	...
	if m.Config.App.MaxProfileLimit > 0 && len(m.Profiles) >= m.Config.App.MaxProfileLimit {
		...
		return nil, fmt.Errorf("实例数量已达上限 (%d个)，无法复制实例", m.Config.App.MaxProfileLimit)
	}
	...
	profile := &Profile{
		...
		AutoProxySwitchEnabled:       src.AutoProxySwitchEnabled,
		AutoProxySwitchGroupName:     src.AutoProxySwitchGroupName,
		AutoProxySwitchMode:          normalizeAutoProxySwitchMode(src.AutoProxySwitchMode),
		AutoProxySwitchIntervalM:     src.AutoProxySwitchIntervalM,
		AutoProxySwitchRotateByGroup: src.AutoProxySwitchRotateByGroup,
		AutoProxySwitchLastProxyId:   src.AutoProxySwitchLastProxyId,
...
func normalizeAutoProxySwitchInterval(minutes int) int {
	if minutes <= 0 {
		return 5
	}
	if minutes < 1 {
...
func normalizeAutoProxySwitchMode(mode string) string {
